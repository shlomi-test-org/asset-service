...
const getTenantGithubRepos = async (tenant) => {
    ...
    // get first page of github installation repos
    let response = await getGithubInstallationReposInfo(tenantInfoResponse.token, tenant, 1, per_page);
    repositories.push(...response.repositories)

    const number_of_pages = Math.ceil(response.total_count / per_page);
    console.log(`*** number_of_pages ${number_of_pages}`);

    // get rest of github installation repos pages
    for (let page = 2; page <= number_of_pages; page++) {
        response = await getGithubInstallationReposInfo(tenantInfoResponse.token, tenant, page, per_page);
        repositories.push(...response.repositories)
    }
    ...
    return {token: tenantInfoResponse.token, repos: repositories}
};
